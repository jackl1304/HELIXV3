...
export function setupCustomerAIRoutes(app: Express) {
  // Customer Analysis API Route (neutralisiert)
  app.get('/api/customer/analysis', async (req, res) => {
    try {
      console.log('[TEMP-ANALYSIS] Customer Analysis endpoint called');
      ...
      const mockInsights = [
        {
          ...
          content: "Analyse zeigt eine 47% Zunahme der FDA-Aktivitäten im Bereich Herzschrittmacher in den letzten 30 Tagen. Dies deutet auf mögliche neue Regulierungen hin.",
      ...
      console.error('[TEMP-ANALYSIS] Error in customer analysis endpoint:', error);
      res.status(500).json({
