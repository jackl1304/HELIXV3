...
export default function RegulatoryUpdateDetailPage() {
  ...
  // Generiere heuristische Risikoanalyse basierend auf spezifischen Update-Daten
  const generateAIAnalysis = (update: RegulatoryUpdate) => {
    const isHighPriority = update.priority === 'high';
    const isApproval = update.update_type === 'approval';
    const title = update.title?.toLowerCase() || '';

    // Spezifische automatisierte Analyse basierend auf Produkttyp
    if (title.includes('inbellamax')) {
      return {
        riskScore: 28,
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="ai" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center space-x-2">
                  ...
                  <span>Risiko- & Erfolgsmetriken (heuristisch)</span>
                </CardTitle>
              </CardHeader>
            ...
            {/* Insights (neutralisiert) */}
            {aiAnalysis.aiInsights && (
              <Card className="lg:col-span-2">
                <CardHeader>
                  <CardTitle className="flex items-center space-x-2">
                    <Brain className="w-5 h-5 text-indigo-600" />
                    <span>Erweiterte Insights & Predictive Analytics</span>
                  </CardTitle>
                </CardHeader>
                <CardContent>
